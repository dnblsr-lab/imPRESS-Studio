...
Complete release history for imPRESS Studio. English section first, Polish detailed source below.

This English part is a clean public changelog based on the detailed Polish release notes.

## 1.3.6 — June 7, 2026
**Status:** Latest · Hotfix
*License activation hotfix*

- Fixed a false “license control file was modified” block when changing licenses on the same machine.
- Added a restart reminder after loading a license key or license file.
- Added admin-panel actions to download `.lic` files and copy license keys.

## 1.3.5 — June 6, 2026
**Status:** Stable
*Machine identifier export to `.lic` and PDF merge tool*

- Added machine identifier export from the license activation window to a `.lic` file.
- Updated the trial-license generator page to accept the exported `.lic` identifier file.
- Added a standalone PDF merge action with generated output names based on date, file count, and page count.

## 1.3.4 — May 24, 2026
**Status:** Stable
*Step & Repeat, marker profiles, localized names*

- Added a dedicated `StepAndRepeat` binding for label, sticker, and repeated-item production.
- Added per-copy rotation patterns, center-to-center pitch mode, validation for overlapping copies, and a new Step & Repeat template section.
- Added localized template preset names and localized built-in marker profile names.
- Introduced Marks v2: reusable marker profiles for crop marks, registration marks, color bars, slug, fold marks, and contour-cut settings.
- Added an in-app marker profile editor and synchronized marker rendering between preview and PDF export.

## 1.3.2 — May 23, 2026
**Status:** Stable
*Full inch support, ICC profile details in File Info, US/ANSI presets*

- Completed mm/inch support across the UI while keeping the internal model in millimeters.
- Added dynamic unit labels, converted numeric template fields, inspector values, status values, and preflight measurements.
- Added embedded ICC profile details to File Info, including profile name and color space.
- Added bundled ICC profile plumbing and documentation for ECI profiles.
- Added ANSI paper presets and more US-oriented template suggestions.

## 1.3.1 — May 21, 2026
**Status:** Stable
*Topbar polish — clearer Plan and Export buttons*

- Added a dedicated visual style for the Plan button so it reads as an action without competing with Export.
- Forced the Export button text to white for consistent contrast.

## 1.3.0 — May 18, 2026
**Status:** Stable
*UI redesign, onboarding, drag-and-drop fix, export progress, OPOS preview*

- Fixed Explorer drag-and-drop by restoring a synchronous STA GUI entry point and adding additional drop safeguards.
- Added a richer empty state with sheet mockup, drop zone, checklist, drag-hover overlay, and shortcut hints.
- Reorganized the floating canvas toolbar into clear zoom, view, side, and CMYK-channel groups.
- Added export progress phases, Ghostscript auto-detection, and clearer success messages for PDF/X output.
- Rendered crop marks, registration marks, and Summa OPOS markers directly on the canvas.
- Improved RollFed centering, margins, duplex suggestions, template suggestions, topbar, step rail, panels, and several UI polish issues.

## 1.2.4 — May 17, 2026
**Status:** Stable
*Context-aware template suggestions, RollFed duplex, units, preview ruler*

- Added context-aware template suggestions that adapt presets to the current sheet, binding, margins, marks, roll, and overlay settings.
- Added RollFed duplex support and improved spread handling for extreme roll/banner aspect ratios.
- Added global display units: metric millimeters or inches.
- Added top and left preview rulers with automatic tick spacing.
- Fixed Reset All so the default template is restored and preview planning works after reloading a file.

## 1.2.3 — May 17, 2026
**Status:** Stable
*Smart preflight, template suggestions, layout optimizer, preview and imposition fixes*

- Added content-based bleed detection and a narrative smart preflight summary.
- Added template suggestions from PDF dimensions and automatic Roll 320 registration.
- Added Auto N-up layout optimization.
- Added trim/bleed guides, manual update checking, and improved duplex pairing for ganging.
- Fixed several preview UX issues including sheet numbering, front/back toggle behavior, spread view, CMYK bar position, and overview thresholds.

## 1.2.2 — May 16, 2026
**Status:** Stable
*Preview pipeline v3 — large roll memory fix and screen-proportional rendering*

- Reworked preview caching to avoid multi-gigabyte memory spikes on A0/A1 and long roll jobs.
- Added memory-budget LRU caching, bitmap scale clamping, display-pixel-aware rendering, cancellation tokens, and lower scale buckets.
- Added vector overview mode for low zoom levels.
- Improved roll-aware sheet layout and forced RollFed into single-sheet preview mode.
- Refactored preview dimension probing, cache insertion, and bitmap rendering APIs.

## 1.2.1 — May 16, 2026
**Status:** Stable
*Template editor UX hardening after first 1.2 tests*

- Preserved template editor state after closing the dialog.
- Added validation that blocks closing invalid templates and shows clearer Polish error messages.
- Added auto-generated template IDs, better defaults, and less duplicate validation noise.
- Added split/continuous roll-length switching and per-section enable toggles.
- Updated website pages and architecture around roll and overlay options.

## 1.2.0 — May 15, 2026
**Status:** Stable
*Roll-fed / print-and-cut production with Summa OPOS markers*

- Added the `RollFed` binding for step-and-repeat production on roll media.
- Added variable repeat, multi-segment splitting, basic nesting, and front-only output.
- Added Summa OPOS / OPOS-XY cutter marks with dedicated layout and renderer components.
- Added logo/overlay support per sheet.
- Extended the template editor, model, validation, composer, and tests for roll-fed production.

## 1.1.1 — May 10, 2026
**Status:** Stable
*Run as User — no Program Files writes, hot-folder UX hardening*

- Moved dynamic application data to `%APPDATA%` / `%LOCALAPPDATA%`.
- Added a central `AppPaths` helper and one-time migration from 1.0.x paths.
- Improved hot-folder defaults, field validation, license status, crash recovery banners, and invalid-license handling.
- Added stronger I/O guard clauses, atomic config writes, config rotation, startup summaries, and cleaner logs.
- Patch release focused on runtime and UX hardening without breaking changes.

## 1.1.0 — May 9, 2026
**Status:** Stable
*Hot Folders — automated production imposition*

- Added a full hot-folder system for production print workflows with multiple folders, file-copy completion detection, SMB/DFS polling fallback, crash recovery, SQLite ledger, deduplication, retries, and Ghostscript process limiting.
- Added a hot-folder management UI, edit dialog, list actions, error banners, and full PL/EN localization.
- Added CLI commands for running, listing, starting, stopping, and reloading hot folders.
- Added configuration validation, long path support, a new `Imposition.HotFolder` module, and production resilience mechanisms.
- Hot folders require a valid license and create their configuration under `%APPDATA%` on first run.

## 1.0.8 — April 26, 2026
**Status:** Stable
*Color & Clock — ICC management and clock rollback protection*

- Added full ICC profile management in the export window.
- Added license protection against system clock rollback.
- Updated the contact email.

## 1.0.7 — April 26, 2026
**Status:** Stable
*License Hardening — strict export gate, new fingerprint, hardware tolerance*

- Fixed unlicensed export access.
- Reworked the hardware fingerprint by removing hostname and using three independent components with 2-of-3 tolerance.
- Added publisher tools: computer identifier in activation, PowerShell license issuing script, and richer CLI fingerprint output.
- Documented that the private RSA key must never be shipped with the app.

## 1.0.6 — April 26, 2026
**Status:** Stable
*Progressive Zoom — immediate preview while zooming*

- Added progressive preview loading similar to map applications.
- Improved zoom bucket coarsening, cancellation during zoom spam, and bitmap-reference cache filtering.
- Documented PDFium/Docnet CPU rasterization as the real preview bottleneck.

## 1.0.5 — April 26, 2026
**Status:** Stable
*Ink Inspector — CMYK channel separation and eyedropper*

- Added CMYK channel toggles.
- Added an eyedropper color sampler.
- Expanded preview options with channel controls and color-coded C/M/Y/K labels.
- Clarified that this is a visual simulation, not true production separations.

## 1.0.4 — April 26, 2026
**Status:** Stable
*Adaptive Preview — faster preview for large PDFs*

- Added parallel page rasterization, neighboring-sheet prerendering, zoom quantization, and an LRU bitmap cache.
- Clarified GPU usage in Avalonia/Skia and prevented RenderAndCache from blocking worker threads indefinitely.
- Fixed strict preflight report handling, a preflight dialog race condition, and installer script details.

## 1.0.3 — April 26, 2026
**Status:** Stable
*Print-Ready — PDF/X-1a, preflight report, preview numbering*

- Added PDF/X-1a:2001 export.
- Added pre-export preflight reporting and visible sheet numbering in preview.
- Expanded preflight validation.
- Replaced the old PDF/X-4 checkbox with a PDF/X standard dropdown and automatic Ghostscript arguments.

## 1.0.2 — April 26, 2026
**Status:** Stable
*Numbering & Persistence — sheet numbering and saved export options*

- Added sheet numbering, Ghostscript file picker, Inno Setup installer, and GitHub Releases auto-update.
- Fixed export option persistence.
- Moved versioning to SemVer, replaced Velopack with Inno Setup, and expanded the export dialog.

## 1.01 — April 19, 2026
**Status:** Stable
*Polish Pass — menu, settings, and gang-run*

- Added the application menu, settings window, i18n infrastructure, and 3+ gang-run templates.
- Moved About and language selection into cleaner menu/settings locations.
- Updated UI text and Polish diacritics.

## 1.00 — April 18, 2026
**Status:** Stable
*First Impress — first official production release*

- Added licensing, activation, app icon, licensing CLI, PDF file info panel, rebranding, and an extended left panel.
- Renamed the executable to `imPRESS Studio.exe` and standardized dialog icons.
- Added RSA-2048 license verification and SHA-256 hardware hashing.

## 0.9 — April 10, 2026
**Status:** Beta
*Release Candidate — final tests before 1.0*

- Added multi-file loading, source PDF merging, file reordering, file info, and Reset All.
- Reworked the source-file panel and selected-file loading flow.
- Fixed encrypted PDF loading freezes and template-name display in the title bar.

## 0.8 — March 28, 2026
**Status:** Beta
*Polish & Stats — UI polish and statistics panel*

- Added statistics panel, numeric controls, progress bars, binding info card, and sheet thumbnails.
- Reworked the header menu and visually separated the main panels.

## 0.7 — March 15, 2026
**Status:** Beta
*Preview Master — interactive sheet preview*

- Added interactive preview, cursor-centered zoom, pan, view rotation, spread mode, creep visualization, overprint simulation, and keyboard shortcuts.

## 0.6 — February 25, 2026
**Status:** Beta
*Print Marks & Creep*

- Added printer marks, creep compensation, folding options, and custom paper sizes.
- Expanded margin support for bleed, safe zone, gutter, spine, and sheet margin.

## 0.5 — February 10, 2026
**Status:** Beta
*PDF/X-4 & Ghostscript*

- Added PDF/X-4 conversion, ICC profile support, strict preflight, sheet ranges, and CLI mode.

## 0.4 — January 20, 2026
**Status:** Beta
*Template Manager*

- Added template manager, JSON template import/export, built-in templates, and template validation.

## 0.3 — January 8, 2026
**Status:** Beta
*Binding Types*

- Added perfect binding, folding, and configurable signatures.

## 0.2 — December 18, 2025
**Status:** Beta
*Saddle Stitch MVP*

- Added the first working saddle-stitch imposition, PDF export, drag-and-drop, and a status bar.

## 0.1 — December 1, 2025
**Status:** Alpha
*Genesis — application skeleton*

- Added the base Avalonia/.NET architecture, project split, PDF loading, DI container, and Serilog logging.

## Plany rozwoju — 2026+
*Roadmap — planned future features*

- Planned: Windows Service mode for hot folders.
- Planned: REST API and webhooks.
- Planned: JDF/JMF integration.
- Planned: macOS and Linux versions.
- Planned: OpenGL preview engine, more UI languages, more gang-run templates, and regex-based multi-template hot-folder routing.
...
Format zgodny z [Keep a Changelog](https://keepachangelog.com/pl/1.1.0/), wersjonowanie [Semantic Versioning](https://semver.org/lang/pl/).

## 1.3.6 — 7 czerwca 2026
**Status:** Najnowsza · Hotfix
